...
{
    public class CustomeRankCustomerDA : BaseDA
    {
        #region Constructer
        public CustomeRankCustomerDA()
        {
        }

        public CustomeRankCustomerDA(string pathPaging)
        {
            PathPaging = pathPaging;
        }

        public CustomeRankCustomerDA(string pathPaging, string pathPagingExt)
        {
            PathPaging = pathPaging;
		...
		public CustomerItem GetCustomerItemById(int name)
		{
        ...
        public CustomerItem GetCustomerByUserName(string name)
        {
            var query = from c in FDIDB.Customers
                        ...
                        select new CustomerItem
                        {
							...
							Level2 = c.Level2??0,
                            TotalPricePv = c.Custome_RankCustomer.Sum(p => p.PricePV),
                        };
            return query.FirstOrDefault();
        }

        public Customer GetCusByUserName(string name)
        {
            var query = from c in FDIDB.Customers
                        where c.UserName.ToLower().Equals(name.ToLower())
                        select c;
		...
		public CustomeRankCustomerItem GetCustomerByRankID(int id)
		{
			var query = from c in FDIDB.Custome_RankCustomer
						where c.IsDeleted == false && c.PricePV > 5 && c.ID == id
						...
						select new CustomeRankCustomerItem
						{
							...
							DateCreated = c.DateCreated,
							PricePV = c.PricePV,
							ProvincialAgenciesName = c.ProvincialAgency != null ? c.ProvincialAgency.Name : "",
							RankCustome = new RankCustomeItem()
							{
								Name = c.RankCustome.Name,
							},
							UserCreated = c.UserCreated,
							Note = c.Note,
							IsDeleted = c.IsDeleted,
							Goi = c.Goi ?? 0,
							NameGoi = c.Goi != null ? c.Goi == 100 ? "Lấy hàng" : c.Goi == 70 ? "Cho nợ" : "Trả nợ" : "",
						};
			return query.FirstOrDefault();
		}

		public List<CustomeRankCustomerItem> GetCustomerById(int id, int take)
		{
			var query = from c in FDIDB.Custome_RankCustomer
						where c.IsDeleted == false && c.PricePV > 5 && c.CustomerId == id
						orderby c.ID descending
						select new CustomeRankCustomerItem
						{
							ID = c.ID,
        ...
        /// <returns>Danh sách bản ghi</returns>
        public List<Custome_RankCustomer> GetListByArrID(List<int> ltsArrID)
        {
            var query = from c in FDIDB.Custome_RankCustomer where ltsArrID.Contains(c.ID) select c;
            return query.ToList();
        }
		...
		public List<Shop_Order_Product> GetShopOrderProductByCustomerRankId(int RankId)
		{
			var query = from c in FDIDB.Shop_Order_Product where c.CustomeRankCustomerId == RankId select c;
			return query.ToList();
		}

		public void DeleteShopProduct(Shop_Order_Product shop_Order_Product)
		{
			FDIDB.Shop_Order_Product.Remove(shop_Order_Product);
		}

		public void AddShopProduct(Shop_Order_Product shop_Order_Product)
		{
			FDIDB.Shop_Order_Product.Add(shop_Order_Product);
		}
        ...
        public void Add(Custome_RankCustomer customeRankCustomer)
        {
            FDIDB.Custome_RankCustomer.Add(customeRankCustomer);
        }
        ...
        public void Delete(Custome_RankCustomer customeRankCustomer)
        {
            FDIDB.Custome_RankCustomer.Remove(customeRankCustomer);
        }
