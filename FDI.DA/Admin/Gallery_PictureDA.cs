...
namespace FDI.DA.Admin
{
    public partial class Gallery_PictureDA : BaseDA
    {
        ...
        /// <param name="httpRequest"> </param>
        /// <returns>Danh sách bản ghi</returns>
        public List<PictureItem> GetListSimpleByRequest(HttpRequestBase httpRequest, string lang, int type = 0)
        {
            Request = new ParramRequest(httpRequest);
            var query = from o in FDIDB.Gallery_Picture
                        where !o.IsDeleted.Value && o.LanguageId.ToUpper().Equals(lang.ToUpper()) && ((type != 0 && o.Type == type) || type == 0)
                        orderby o.DateCreated descending
                        select new PictureItem
                        {
                            ID = o.ID,
