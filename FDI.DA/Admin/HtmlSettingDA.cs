...
namespace FDI.DA.Admin
{
    public class HtmlSettingDA : BaseDA
    {
        ...
        {
            var query = from c in FDIDB.HtmlSettings
                        where c.Key.ToLower().Equals(key.ToLower()) && c.LanguageId.ToLower().Equals(LanguageId)
                        select c;
        ...
        {
            var query = from c in FDIDB.HtmlSettings
                        where c.Key.ToLower().Equals(key.ToLower()) && c.url.ToLower().Equals(url.ToLower()) && c.LanguageId.ToLower().Equals(LanguageId)
                        select c;
            return query.FirstOrDefault();
        }
        ...
        public List<HtmlSettingItem> GetListSimpleByRequest(HttpRequestBase httpRequest)
        {
            ...
            var query = from o in FDIDB.HtmlSettings
                        where o.LanguageId.ToLower().Equals(LanguageId)
        ...
        {
            var query = from o in FDIDB.HtmlSettings
                        where o.ID == id && o.LanguageId.ToLower().Equals(LanguageId)
